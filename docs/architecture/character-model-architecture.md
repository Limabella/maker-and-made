...
- `dex`, `historical_hall`, and `literary_hall` are private-first.
...
- `historical_hall` and `literary_hall`: Level 1 to Level 2
...
`experimental_lab` is the public lab, `dex` is the deep research layer, and `historical_hall` plus `literary_hall` are private low-risk pipeline and agent zones.
