...
이 실험은 Kanana, EXAONE, Qwen이 한국어 대화에서 표현된 감정, 발화 행위, 관계 신호와 문맥을 얼마나 안정적으로 구조화하는지 비교한다. 분류 성능만 비교하지 않고, 분석 결과가 ONN-C State Engine의 변화에 미치는 영향과 로컬 실행 비용도 함께 평가한다.
...
모델 가중치는 Git 저장소 루트에 저장하지 않는다. Ollama 전용 모델 저장소를 사용하고, 프로젝트에는 다음 재현 정보만 기록한다.

- 모델의 정확한 이름과 버전
- 원본 배포 페이지와 변환 파일 출처
- 파라미터 수와 context window
- 양자화 형식과 파일 크기
- GGUF 파일의 SHA-256 해시
- Ollama 및 GPU 드라이버 버전
- 적용한 `Modelfile`과 생성 옵션

Kanana GGUF처럼 커뮤니티 변환본을 사용하는 경우에는 변환 저장소, 정확한 파일명과 해시를 반드시 기록한다. 대용량 모델 파일과 벤치마크 원시 출력은 `.gitignore`로 제외하고, 집계 결과와 실험 설정만 버전 관리한다.

## 모델 비교 요약

| 단계 | 비교 내용 | 주요 조건 및 지표 |
|---|---|---|
| 모델 조건 | Keyword baseline, Kanana 2.1B, EXAONE 2.4B, Qwen3 1.7B 및 품질 상한용 Qwen3 4B | 언어, context, license, BF16/Q4_K_M, 동일 prompt와 JSON Schema |
...
4. 크기가 다른 Qwen3 4B의 결과는 1.7B와 동일 선상의 승패가 아니라 품질-비용 상한으로 해석한다.
...
Kanana는 한국어 중심 모델이므로 일상적인 한국어의 말투, 생략, 높임말과 관계적 표현에서 강점을 보일 가능성이 있다. 특히 직접적인 감정 단어가 없어도 “그냥 더 말하기 싫어”, “됐어, 신경 쓰지 마”처럼 대화를 중단하려는 표현을 철회 또는 거리 두기 신호로 포착하는지가 핵심이다. 반면 커뮤니티 GGUF 변환본을 사용할 경우 원본 모델과 양자화 모델 사이의 성능 차이 및 재현성 관리가 필요하다.

### EXAONE 3.5 2.4B

EXAONE은 한국어와 영어를 함께 다루는 비교적 작은 모델로, 현재 로컬 장비에서 성능과 실행 비용의 균형 후보가 된다. 한국어 복합 문장과 명시적인 책임 인정, 사과 및 관계 회복 계획을 얼마나 일관된 JSON으로 변환하는지가 주요 평가 대상이다. 모델 라이선스는 연구 사용과 향후 배포 조건을 분리하여 검토해야 한다.

### Qwen3 1.7B

Qwen3 1.7B는 Kanana 및 EXAONE과 크기가 비슷하므로 size-matched comparison에 사용한다. 다국어 모델이 작은 파라미터 규모에서도 한국어 반어, 완곡한 거절과 관계 신호를 어느 정도 유지하는지 확인한다. 만약 속도와 출력 안정성은 우수하지만 한국어의 미묘한 관계 표현에서 성능이 낮다면, 빠른 보조 분석기 또는 경량 baseline으로서의 가능성을 별도로 해석할 수 있다.

### Qwen3 4B

Qwen3 4B는 동일 크기 비교 대상이 아니라 품질-비용 상한을 확인하는 모델이다. 긴 문맥이나 혼합 신호에서 1.7B 모델보다 나은 결과를 보이는지, 그 차이가 증가한 VRAM과 latency를 정당화하는지를 평가한다. 4B가 정확도에서 앞서더라도 로컬 운영 비용 대비 개선 폭이 작다면 최종 배포 모델로 선택하지 않을 수 있다.
...
- [ ] Kanana GGUF 출처, 파일명과 SHA-256을 기록한다.
